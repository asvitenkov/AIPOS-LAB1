...
BOOL WINAPI NetworkResources::EnumerateFunc(LPNETRESOURCE lpnr)
{
    ...
    if (dwResult != NO_ERROR) {
        // ошибка WNetOpenEnum нужно попытаться обработать
...
void NetworkResources::errorHandler(DWORD errorCode, QString dsc){
    ...
    if(dwWNetResult==NO_ERROR){
        ...
        if(!describing.isEmpty()) msg+="\n"+QString::fromLocal8Bit("     описание: ")+describing;
        emit sendMessage(msg);
    }
    else emit sendMessage(QString::fromLocal8Bit("errorHandler ошибка ")+QString::number(dwWNetResult));
